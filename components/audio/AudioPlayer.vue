<template>
  <div>
    ...
    <playlist v-if="showPlayer && showPlaylist"/>
    <transition name="player-from-bottom">
      <div 
        v-show="showPlayer && !minimized" 
        id="audioplayer">
        <div class="audioplayer d-none d-md-flex">
          <div class="ap__element button-box audio-control">
            <div class="audio-control__buttons">
              <backward-button 
                class="audio-control__btn" 
                @backward="backwards()"/>
              <play-button 
                ref="playBtn" 
                :release="currentTrack && currentTrack.release" 
                class="audio-control__btn play" 
                background="#30C46C"
                @play="onPlay" 
                @pause="onPause"/>
              <forward-button 
          ...
          <div 
            class="ap__element button-box link-box" 
            @click="onBurgerClick">
          ...
          <div 
            class="ap_element button-box link-box" 
            @click="onClose">
            <div class="close-playlist"/>
          </div>
        </div>
        <div 
          class="audioplayer d-sm-flex d-md-none" 
          @touchstart="startAudioTouch" 
          @touchend="endAudioTouch">
          <div class="controls flex-row d-flex">
            ...
            <div 
              :class="['ap__element', 'd-flex', 'playlist-button', showPlaylist ? 'close-arrow' : '']" 
              @click="onBurgerClick">
              <template v-if="!showPlaylist">
                <div class="burger-menu"/>
              </template>
            ...
            <div 
              class="ap_element d-flex button-box close-playlist-box" 
              @click="onClose">
              <div class="close-playlist"/>
            </div>
            <!--</div>-->
          ...
          <div 
            v-if="currentTrack && currentTrack.release" 
            class="current-track current-track__info-box d-flex flex-row justify-content-between">
            <nuxt-link 
              :to="{name: 'releases-slug', params: {slug: currentTrack.release.slug}}" 
              class="release-image">
    ...
    <transition name="player-from-bottom">
      <div 
        v-show="showPlayer && minimized" 
        ref="minified" 
        class="minified-controls d-sm-block d-md-none">
        <div 
          v-if="currentTrack" 
          class="release-marquee" 
          @click="maximize">
...
export default {
  ...
  computed: {
    ...
    showPlayer() {
      ...
      return (
        this.visible &&
        (name === null ||
          (!['checkout', 'cart'].includes(name) && !name.startsWith('account')))
      )
    },
    playlistPos() {
  ...
  methods: {
    ...
    onBurgerClick() {
      this.$store.commit(
        types.SET_PLAYLIST_VISIBLE,
        !this.$store.state.player.playlistVisible
      )
    },
    onClickaway() {
...
.player-from-bottom-enter,
.player-from-bottom-leave-to {
  margin-bottom: -100%;
}
