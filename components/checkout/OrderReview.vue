<template>
  <div class="checkout__review">
    ...
    <checkout-overview class="checkout__overview"/>
    <cart-content :review="true"/>
    <primary-control-panel class="d-sm-flex d-md-none"/>
    <div 
      v-show="placingOrder" 
      class="placing-order">
  ...
  methods: {
    onPlaceOrder() {
      if (!this.$store.getters.isAuthenticated) {
        ...
        this.$store.dispatch('addAlert', {
          level: 'error',
          message: 'You must agree to the terms of conditions to proceed.'
        })
      } else if (
        (typeof this.porto === 'undefined' ||
          this.porto === parseFloat('0.00')) &&
        (!this.isSelfCollector && !this.isOnlyPresale)
      ) {
        this.$store.dispatch('addAlert', {
          level: 'error',
          message: 'You must specify a shipping option.'
        })
      } else {
        ...
        this.$apollo
          .mutate({
            ...
            variables: {
              ...
              paymentMethodId:
                this.selectedPaymentMethod && this.selectedPaymentMethod.id,
          ...
          .then(({ data }) => {
            ...
            if (data.placeOrder.paymentUrl) {
              ...
              if (
                !order.isPaid &&
                !(order.isSelfCollector && order.paymentType === 'cash')
              ) {
                ...
                this.$store.commit(types.ADD_ALERT, {
                  level: 'info',
                  message:
                    'Your order has been placed. Please fulfill order with payment.'
                })
              } else {
                ...
                if (notInStock && notInStock.length > 0) {
                  for (var line = 0; line < notInStock.length; line++) {
                    ...
                    this.$store.commit(types.ADD_ALERT, {
                      level: 'warning',
                      message: `We are sorry, but the last copy of '${
                        backorderLine.release.name
                      } - ${
                        backorderLine.release.title
                      }' was just grabbed. Ordered ${
                        backorderLine.quantity
                      } copy${backorderLine.quantity > 1 ? 's' : ''} for you.`
                    })
                  }
                }
