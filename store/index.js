  ...
  return new Vuex.Store({
    ...
    mutations: {
      ...
      [types.SET_USER_SHIPPING_ADDRESSES]: (state, args) => {
        ...
        if (
          args.shippingAddresses.length > 0 &&
          (!shippingAddress || isEmptyAddress(shippingAddress))
        ) {
      ...
      [types.SET_USER_BILLING_ADDRESSES]: (state, args) => {
        ...
        if (
          args.billingAddresses.length > 0 &&
          (!billingAddress || isEmptyAddress(billingAddress))
        ) {
    ...
    getters: {
      ...
      isShippingAddressConfirmed(state) {
        return (
          state.checkout.shipping.address && state.checkout.shipping.confirmed
        )
      },
      ...
      getShippingAddress(state) {
        return (
          ...
          (state.userProfile.shipping.addresses &&
            state.userProfile.shipping.addresses.length > 0 &&
            state.userProfile.shipping.addresses[0])
        )
      },
      getBillingAddress(state) {
        return (
          state.checkout.billing.address ||
          (state.userProfile.billing.addresses &&
            state.userProfile.billing.addresses.length > 0 &&
            state.userProfile.billing.addresses[0])
        )
      },
      ...
      getCheckoutState(state, getters) {
        ...
        if (
          (checkoutState === 1 && state.user.authenticated) ||
        ...
        if (
          (checkoutState === 2 && getters.isShippingAddressConfirmed) ||
          (getters.isOnlyPresale && checkoutState > 1)
        ) {
          checkoutState = 3
        }

        if (
          (checkoutState === 3 && getters.isPaymentOptionConfirmed) ||
          (state.checkoutActive && getters.isOnlyPresale && checkoutState > 1)
        ) {
          checkoutState = 4
        }
      ...
      isEmptyCart: (state, getters) => {
        return (
          getters.getCart &&
          (getters.getCart.lines.length === 0 &&
            getters.getCart.preorderLines === 0)
        )
      },
