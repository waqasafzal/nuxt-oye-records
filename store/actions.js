...
import {
  callArtistSearchQuery,
  callLabelSearchQuery,
  callReleaseSearchQuery
...
function runPasswordValidation(commit, password, passwordConfirm) {
  ...
  if (!password || password.length < 8) {
    ...
    commit(
      types.SET_USER_FORM_PWD_ERROR,
      'Password must contain at least one digit and one of these special chars: !#$%^&+=?'
    )
    ok = false
  } else {
  ...
  if (password && password !== passwordConfirm) {
    commit(
      types.SET_USER_FORM_PWD_CONFIRM_ERROR,
...
export const getCart = function({ commit, dispatch }) {
  ...
  new Promise ((resolve, reject) => {
    ...
    .then (({data}) => {
      const cart = data.syncCart.cart
      if (cart.cookie) {
        Vue.cookie.set ('cart', cart.cookie, true)
      }
      dispatch ('setCart', {cart: cart})
      const addedItems = data.syncCart.addedItems
      if (addedItems) {
        for (let i = 0; i < addedItems.length; i++) {
          ...
          commit (types.ADD_ALERT, {
            ...
            message: `Release '${release.name} - ${
              release.title
              }' is on stock again. Added ${item.quantity} item${
              item.quantity > 1 ? 's' : ''
              } to order.`
          })
        }
      }
      const removedItems = data.syncCart.removedItems
      if (removedItems) {
        for (let i = 0; i < removedItems.length; i++) {
          ...
          commit (types.ADD_ALERT, {
            ...
            message: `Release '${release.name} - ${
              release.title
              }' does not have enough items on stock. Moved ${
              item.quantity
              } item${item.quantity > 1 ? 's' : ''} for preorder.`
          })
        }
      }
      return resolve (cart)
...
export const addToCart = function({ commit, dispatch }, args) {
  new Promise ((resolve, reject) => {
...
export const updateCart = ({ commit, dispatch }, args) =>
  new Promise((resolve, reject) => {
...
export const nextTrack = ({ commit }) =>
  new Promise((resolve, reject) => {
    commit(types.PLAY_FORWARD)
  })

export const prevTrack = ({ commit }) =>
  new Promise((resolve, reject) => {
    commit(types.PLAY_BACKWARD)
  })

export const setUser = ({ commit }, args) =>
  new Promise((resolve, reject) => {
    commit(types.SET_USER, args.user)
  })

export const addAlert = ({ commit }, args) =>
  new Promise((resolve, reject) => {
    commit(types.ADD_ALERT, args)
  })

export const search = function({ commit }, args) {
  new Promise((resolve, reject) => {
    const client = this.app.apolloProvider.defaultClient
    let type = args.type
    let mutationType = args.append
      ? types.ADD_SEARCH_RESULTS
    ...
    if (query) {
      ...
      if (type === 'releases') {
        ...
        callReleaseSearchQuery(
          ...
          args.size,
          1,
          JSON.stringify(['cat_no']),
          ({data}) => {
            let searchResults = data.search
            commit(types.SET_CATNO_RESULTS, {
              search: searchResults,
              type: type,
              head: true
            })
        ...
        callLabelSearchQuery(client, query, args.size, ({data}) => {
          let searchResults = data.search
          commit(mutationType, {search: searchResults, type: type})
...
export const getProfile = function(store, args) {
  new Promise ((resolve, reject) => {
    const client = this.app.apolloProvider.defaultClient
    client.query ({
      query: gql`
    ...
    .then (({data}) => {
      ...
      if (announcements && announcements.length > 0) {
        ...
        store.commit (
          types.SET_ANNOUNCEMENTS,
          announcements.filter (info => !msgList.includes (info.message))
        )
      }
      resolve (profile)
    })
...
export const enterCheckout = (store, args) =>
  new Promise((resolve, reject) => {
...
export const setShippingCountry = function(store, args) {
  new Promise ((resolve, reject) => {
    const client = this.app.apolloProvider.defaultClient
    client.query ({
      query: gql`
      ...
      store.commit (
        types.SET_SHIPPING_OPTIONS,
        data.cart.shippingOptions.options
      )
      store.commit (
...
export const validateUserForm = ({ commit }, args) =>
  new Promise((resolve, reject) => {
    let user = args.user
    let ok = true
    ...
    if (typeof user.firstName !== 'undefined') {
      if (user.firstName === '') {
        commit(
          types.SET_USER_FORM_FIRST_NAME_ERROR,
    ...
    if (typeof user.lastName !== 'undefined') {
      if (user.lastName === '') {
        commit(
          types.SET_USER_FORM_LAST_NAME_ERROR,
    ...
    ok =
      ok && runPasswordValidation(commit, user.password, user.passwordConfirm)
    ok = ok && runEmailValidation(user.email, commit)

    return resolve(ok)
  })

export const createNewUser = function({ commit }, args) {
  new Promise((resolve, reject) => {
    let user = args.user
    ...
    .then(({ data }) => {
      if (data.registerUser.errorStatus) {
        const message = `Can not register user ${
          data.registerUser.errorStatus
        }`
        commit(types.ADD_ALERT, { message: message, level: 'error' })
        console.error(message)
      } else {
        commit(
          types.SET_SHIPPING_ADDRESS_ID,
          data.registerUser.userProfile.preferredShippingAddress.id
        )
        commit(
          types.SET_BILLING_ADDRESS_ID,
          data.registerUser.userProfile.preferredBillingAddress.id
        )
...
export const saveAddresses = (store, args) =>
  new Promise((resolve, reject) => {
    ...
    if (billingChanged && shippingChanged) {
      ...
      addressDict['isBilling'] = true
      addressId =
        billingAddress.id !== shippingAddress.id ? billingAddress.id : undefined
      callSaveAddress(addressId, addressDict, ({ data }) => {
...
export const validateAddress = (store, args) =>
  new Promise((resolve, reject) => {
...
export const saveChart = function({ commit }, args) {
  new Promise((resolve, reject) => {
    this.app.apolloProvider.clients.defaultClient.mutate({
      mutation: gql`
        mutation($charts: JSONString!, $artistId: Int) {
...
export const cancelOrder = function({ commit, dispatch }, args) {
  new Promise ((resolve, reject) => {
    this.app.apolloProvider.clients.defaultClient.mutate ({
      mutation: gql`
          mutation($orderId: ID!) {
...
export const setCart = ({ commit }, args) =>
  new Promise((resolve, reject) => {
    var cart = args.cart
...
export const emailValidation = ({ commit }, args) =>
  new Promise((resolve, reject) => {
    resolve(runEmailValidation(args.email, commit))
  })

export const passwordValidation = ({ commit }, args) =>
  new Promise((resolve, reject) => {
    resolve(runPasswordValidation(commit, args.password, args.passwordConfirm))
  })

export const getCountries = function({ commit, state }, args) {
  new Promise ((resolve, reject) => {
    if (!state.countries) {
      this.app.apolloProvider.clients.defaultClient.query ({
        query: gql`
...
export const removeBackOrder = function({ commit, state }, args) {
  new Promise ((resolve, reject) => {
    this.app.apolloProvider.clients.defaultClient.mutate ({
      mutation: gql`
...
export const removeReservation = function({ commit, state }, args) {
  new Promise ((resolve, reject) => {
    this.app.apolloProvider.clients.defaultClient.mutate ({
      mutation: gql`
...
export const subscribeNewsletter = function({ commit }, args) {
  new Promise ((resolve, reject) => {
    this.app.apolloProvider.clients.defaultClient.mutate ({
      mutation: gql`
...
export const updateUser = function({ commit, dispatch }, args) {
  new Promise ((resolve, reject) => {
    this.app.apolloProvider.clients.defaultClient.mutate ({
      mutation: gql`
...
export const sendTransaction = ({ commit }, order) =>
  new Promise((resolve, reject) => {
